...
| `pals_combat.json` | Массив 299 объектов: `name, paldeck_number, elements[], rarity, hp, melee_attack, shot_attack, defense, support, stamina, price, ride, mount_type (ground/flying/swim/null), movement{walk,run,sprint}, size (XS–XL), transport_speed (скорость ПЕРЕНОСКИ груза — отдельный от run стат, 147 транспортников; сборщик `collectors/fetch_transport_speed.py`; у нетранспортных null), swim_speed/swim_dash_speed (скорость плавания/рывка вплавь — у 9 плавательных маунтов; в воде решает dash, не сухопутный спринт; сборщик `collectors/fetch_swim_speed.py`), partner_skill{name,effect}, notable_drops[], role_tags[]`. |
...
| `bosses.json` | `tower_bosses[]` (9 башен 1.0), `raid_bosses[]` (АЛТАРНЫЕ рейды — на базе, бьются назначенные базовые палы + турели) и `world_bosses[]` (мировые/квестовые боссы — пати из 5, напр. **Panthalus** призывом Echoing Flute; НЕ рейд): элементы, каунтер-элементы, уровни, тактика, флаг `preliminary`. Различие категорий — в `categories_note`. |
...
| `passives.json` | `passives[{name, tier, effects, category, exclusive_source[], breedable, mutation_exclusive?, notes?}]` — полный список пассивок, включая вредные. Мутационные (радужные) — ровно 5: Immortality, Idiosyncratic, Babysitter, Heavily Armored, Skymarcher (`mutation_exclusive:true`); первично только из мутационного яйца (Cake 1%/Vegetable 2%/Extravagant 3%), но `breedable:true` — дальше наследуются обычным бридингом (Special Cake повышает шанс); с Yakumo Birds of a Feather не работают. «Skirmisher» не существует (удалён как фантом ранних сводок). |
...
| `combat_traits.json` | Боевые ТРЕЙТЫ (пассивки) бойца, отдельно `party` (ты управляешь → офенс, стекло-пушка ок) и `raid` (базовый рейд — палы дерутся САМИ → выживаемость): у каждого `role`, `simple[]` (графт на Pal Surgery Table / фарм), `complex[]` (радужные World Tree t5 / мутации), `alternatives{}` (замены радужным). `surgery_table` поясняет: простые вживляются Surgery Chip (~10–50k золота), World Tree — только ловля/яйцо. Имена сверены с passives.json. Используется в web-«Каунтерах» (`traitsCardHTML`) и CLI `party combat`/`boss` (`_print_combat_traits`). |
...
- `breeding_mechanics.md` — cake-рецепты, наследование пассивок, IV, Condenser (1.0: 4/8/12/24 = 48 копий-корма до 4★, было 116; мутант 2★ → остаток 36; макс за один синтез, прогресс копится), Mutation и Awakening (новое в 1.0), лучшие пассивки с цифрами.
- `boss_tower_counters.md` — таблица стихий, 9 башенных боссов 1.0 с каунтерами и тактикой, alpha/dungeon/raid-боссы.
- `boss_fighting.md` — рейд ≠ босс (рейд идёт НА БАЗЕ, бьются базовые палы + турели); как строить боевую базу под алтарный рейд; ТОП-3 пала для файтинга боссов + идеальные скиллы; мета-стак боевых пассивок; каунтеры ключевых рейд-боссов. PRELIMINARY 1.0.
...
[--speed] [task1,task2 = И]    handiwork, gathering, lumbering, mining, medicine, cooling,
                              transporting, farming). Несколько задач через запятую = условие И
                              (пал умеет ВСЕ). --speed: сортировать по скорости ПЕРЕНОСКИ
                              (transport_speed — отдельный стат, ГЛАВНЫЙ для транспорта; уровень
                              качается книгами/4★ = сколько несёт, НЕ скорость). Показывает размер
                              (XS–XL) и перенос/бег. Веб-аналог — вкладка «Палы» (единый браузер:
                              поиск + фильтры стихии-И/размер/маунт/🌙/работы-И + сортировки № /
                              🔥Firepower (экспедиции, = (ATK+DEF+HP/5)×(звёзды+1)²) / статы /
                              скорости / CombiRank; бывшая вкладка «Рабочие» влита сюда, #workers → алиас).
                              NB: высокий run ≠ хороший транспортник (Mimog run 2000 / перенос 450).
