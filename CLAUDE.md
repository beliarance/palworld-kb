...
(скорость ранча/плантаций — `--ranch-rate`, `--plant-yield`, `--cook-rate`; выработка Ancient Farm — `--ancient-farm-yield`, по умолчанию 4: калибровка по игроку — 4 фермы кормят 50 палов И 4 фермы = Extravagant торт на 1 хатчери). Торты — не линейные тиры: у каждого свой эффект/станция (Vegetable=2 яйца, Special=пассивки, Extravagant=мутация, Mushroom=статы) — пресет это поясняет.
