...
| `items.json` | `items[]`: `name, category, tech_level, recipe{station, materials{}}` или null, `obtained_from[]`, `notes`, `effect` (у 84 аксессуаров, с paldb; сборщик `collectors/fetch_accessory_effects.py`); плюс `stations[]`. Компоненты рецептов резолвятся по именам внутри же файла. |
