...
(скорость ранча/плантаций — `--ranch-rate`, `--plant-yield`, `--cook-rate`; выработка Ancient Farm — `--ancient-farm-yield`, по умолчанию 3: калибровка по игроку — 4 древние фермы кормят 50 палов с избытком). Торты — не линейные тиры: у каждого свой эффект/станция (Vegetable=2 яйца, Special=пассивки, Extravagant=мутация, Mushroom=статы) — пресет это поясняет.
