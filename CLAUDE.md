...
| `items.json` | `items[]`: `name, category, tech_level, recipe{station, materials{}}` или null, `obtained_from[]`, `notes`; плюс `stations[]`. Компоненты рецептов резолвятся по именам внутри же файла. |
...
| `base_building.json` | Базостроение: 58 структур (tech, ancient-флаг, материалы, worker, power, energy_per_sec), 56 блюд (nutrition/sanity/баффы/спойл), `pal_food_amount` 299/299 (аппетит 1–10), `rates` (яйцо 5 мин, 1 торт = 1 яйцо, таблицы скорости работы по уровням 1–10, механика уровней 9–10: Applied Handbook +1/конденсер/ауры). |
...
(скорость ранча/плантаций — `--ranch-rate`, `--plant-yield`, `--cook-rate`).
