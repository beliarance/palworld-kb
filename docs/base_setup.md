...
**Трейты транспортника** — скорость передвижения, НЕ work-speed: work-бонусы
(Serious/Artisan) на переноску **не влияют** (game8). Три больших: **Swift +30% ·
Runner +20% · Legend +20%** (складываются). `Dimensional Leap` +50% — но +голод.
**Nimble +10% брать не стоит** — слишком слабо; 4-й слот лучше под **сустейн**.

**Аптайм 24/7 — это СТИХИЯ, а не трейт.** **Dark-палы ночные** (73/73 Dark = nocturnal):
работают на базе **круглосуточно, без сна и кроватей** — не нужен трейт Insomnia. Минус:
раз не отдыхают, **быстрее тратят SAN и голод** → дай им **Workaholic** (SAN −15%) /
**Diet Lover** (голод −15%) и поставь **Hot Spring** + сытную еду. Трейт **Insomnia** нужен
только чтобы заставить работать ночью НЕ-Dark пала. game8 советует транспортникам move-speed
**И** food/sanity-трейты — последние как раз чтобы Dark-рабочий не выгорал.

**Work-speed трейты дают МНОГО:** полный стек (Artisan +50% + Remarkable Craftsmanship +75%
+ Work Slave +30%) ≈ **×2.55** к скорости — это сильнее любой разницы в стартовом уровне
работы (уровень всё равно качается книгами). Поэтому при выборе рабочего трейт-потенциал и
стихия (Dark = 24/7) важнее, чем ±1 стартового уровня навыка.
