...
class Planner:
    ...
    def preset_breeding(self):
        ...
        if hatchery:
            self.notes.append("Ancient Hatchery = брид-ферма + ВСТРОЕННЫЙ инкубатор в одном: инкубация ~10 сек "
                              "(почти мгновенная), отдельные Egg Incubator не нужны. "
        ...
        if hatchery:
            self.notes.append(f"Ферм: {farms}. При Ancient Hatchery (встроенный инкубатор + мгновенная инкубация) "
                              "смысла держать больше 1 практически нет: одна и так сыплет яйца быстрее, чем разбираешь "
                              "потомство. 2-ю ставь ТОЛЬКО если реально гоняешь 2 РАЗНЫЕ пары одновременно (--farms 2)")
        else:
